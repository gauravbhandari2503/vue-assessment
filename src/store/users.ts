  ...
  getFilteredData: (state: RootState) => {
    ...
    if (state.searchQuery.length) {
      if (state.searchByFilter === 'all') {
              ...
              if (typeof user[key as keyof User] === 'object') {
                for(const attribute in user[key]) {
                  if (user[key].hasOwnProperty(attribute)) {
                    ...
                    if (cellValue.includes(state.searchQuery)) {
                      return true;
                    }
                  }
                }
        ...
        filteredData = state.users.filter(user => {
          if (user.hasOwnProperty(state.searchByFilter)) {
            let cellValue = '';
            if (typeof user[state.searchByFilter as keyof User] === 'object') {
              for (const attribute in user[state.searchByFilter]) {
                if (user[state.searchByFilter].hasOwnProperty(attribute)) {
                  cellValue = user[state.searchByFilter][attribute].toString().toLowerCase();
                  if (cellValue.includes(state.searchQuery)) {
                    return true;
                  }
                }
              ...
              cellValue = user[state.searchByFilter].toString().toLowerCase();
              if (cellValue.includes(state.searchQuery)) {
                return true;
              }
            }
  ...
  getSearchByCategories: (state: RootState) => {
    if (state.users.length) {
      ...
      for(const key in state.users[0] ) {
        if (key !== 'id') categories.push({id: key, category: key.charAt(0).toUpperCase() + key.slice(1) });
      }
      return categories;
    }
    return []
